...
sub GetTargetTableViewInfo {
	my ($field_name, $asse_or_desc,$tmp_database, $tmp_table,$query_database,$tmp_dir,$toolSelection,$priority_def) = @_;#field_name = anything but not mirbase related which indicate the database which it should query ,ie: mirna_human_strict#tmp_dir is the directory of sql files and result files
    my $output_file=$tmp_dir."/query_output.txt";
	...
	if ($field_name=~'mirbase') {
		...
		$statement = "SELECT m.* FROM $query_database m, $tmp_database.$tmp_table t WHERE t.gene = m.refseq_id AND m.priority >= $priority_def ORDER BY m.$ranked_by $asse_or_desc";
	}
