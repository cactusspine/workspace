...
use constant ID_LOAD_STATEMENT => "SELECT * FROM human_chemicals WHERE id=?";
...
sub GetChemicalIdsForGene {
	my $gene_id = shift;
	 my @ids = ();
	if ($gene_id) {
		my $query =  "SELECT id FROM human_chemicals WHERE gene=? AND relevance>=50";
...
#  	$select->execute($gene_id);
    ...
    $select->execute($gene_id) or die "unable to execute" . $databaseHandle->errstr;
	        my $nr = $select->rows();
        	while ($nr--) {
...
sub GetChemicalDetailsForGene {
 ...
 #       my @details = ();
	my %details = ();
        if ($gene_id) {
                my $query =  "SELECT C.name, C.category, C.relevance, D.bio_element_name FROM human_chemicals C, human_chemical_details D WHERE C.gene=? AND C.relevance>=50 AND C.bioentity_id=D.bioentity_id AND D.biological_db_id=107 order by C.relevance desc;";
                my $databaseHandle = DbSession::GetConnection();
#               my $select = $databaseHandle->prepare($statement);
#               $select->execute($gene_id);
    						my $select = $databaseHandle->prepare($query) or die "unable to prepare query : $query" . $databaseHandle->errstr;
...
#print "loadStatement : $loadStatement\n";
	    carp "Initializing human_chemicals : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
