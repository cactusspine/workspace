package DbEffSummarySheet;

##########################################
##
## Document   : DbEffSummarySheet.pm
...
use constant ID_LOAD_STATEMENT => "SELECT * FROM eff_summary_sheet WHERE id=?";

use constant GENE_LOAD_STATEMENT => "SELECT * FROM eff_summary_sheet WHERE gene=?";
...
sub GetIdForHugo {
        my $hugo_name = shift;
        my $id = "";
        my $statement =  "SELECT id FROM eff_summary_sheet where hugo_name=?";
...
sub _initialize {
	...
	if ($select->rows == 1) {
	    ...
	    carp "Initializing eff_summary_sheet : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
