package DbGpcrChemicalDetails;

##########################################
##
## Document   : DbGpcrChemicalDetails.pm
...
use constant ID_LOAD_STATEMENT => "SELECT * FROM gpcr_chemical_details WHERE id=?";
...
sub GetChemicalDetailsForBioentityId {
	my $bioentity_id = shift;
	 my @ids = ();
	if ($bioentity_id) {
		my $statement =  "SELECT id FROM gpcr_chemical_details WHERE bioentity_id = ?";
...
#print "loadStatement : $loadStatement\n";
	    carp "Initializing gpcr_chemical_details : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
