package DbGprotPathwayReactome;
...
use constant LOAD_STATEMENT => "SELECT * FROM gprot_pathway_reactome WHERE id=?";
...
sub GetAllPathwayReactomeIds {
    ...
    my @reactome_ids = ();
        ...
        my $select = $databaseHandle->prepare("SELECT id FROM gprot_pathway_reactome");
        $select->execute();
	my $nr = $select->rows();
        while ($nr--) {
...
sub _initialize {
	...
	if ($select->rows == 1) {
	    ...
	    carp "Initializing gprot_pathway_reactome : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
