package DbEffChemicals;

##########################################
##
## Document   : DbEffChemicals.pm
...
use constant ID_LOAD_STATEMENT => "SELECT * FROM eff_chemicals WHERE id=?";
...
sub GetChemicalIdsForGene {
	my $gene_id = shift;
	 my @ids = ();
	if ($gene_id) {
		my $statement =  "SELECT id FROM eff_chemicals WHERE gene=? AND relevance>=50";
...
sub GetChemicalDetailsForGene {
 ...
 #       my @details = ();
	my %details = ();
        if ($gene_id) {
                my $statement =  "SELECT C.name, C.category, C.relevance, D.bio_element_name FROM eff_chemicals C, eff_chemical_details D WHERE C.gene=? AND C.relevance>=50 AND C.bioentity_id=D.bioentity_id AND D.biological_db_id=107 order by C.relevance desc;";
                my $databaseHandle = DbSessionGpdb::GetConnection();
                my $select = $databaseHandle->prepare($statement);
                $select->execute($gene_id);
                my $nr = $select->rows();
...
#print "loadStatement : $loadStatement\n";
	    carp "Initializing eff_chemicals : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
