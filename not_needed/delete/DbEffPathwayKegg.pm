package DbEffPathwayKegg;

##########################################
##
## Document   : DbEffPathwayKegg.pm
...
use constant LOAD_STATEMENT => "SELECT * FROM eff_pathway_kegg WHERE id=?";
...
sub GetPathwayNameForKeggId {
	...
	my $select = $databaseHandle->prepare("SELECT DISTINCT pathway_description FROM eff_pathway_kegg WHERE kegg_id=?");
...
sub GetAllPathwayKeggIds {
    ...
    my @kegg_ids = ();
        my $databaseHandle = DbSessionGpdb::GetConnection();
        my $select = $databaseHandle->prepare("SELECT id FROM eff_pathway_kegg");
        $select->execute();
	my $nr = $select->rows();
        while ($nr--) {
...
sub _initialize {
	...
	if ($select->rows == 1) {
	    ...
	    carp "Initializing eff_pathway_kegg : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
