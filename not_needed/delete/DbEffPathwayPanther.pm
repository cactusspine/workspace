...
use constant LOAD_STATEMENT => "SELECT * FROM eff_pathway_panther WHERE id=?";
...
sub GetAllPathwayPantherIds {
    ...
    my @panther_ids = ();
        ...
        my $select = $databaseHandle->prepare("SELECT id FROM eff_pathway_panther");
        $select->execute();
	my $nr = $select->rows();
        while ($nr--) {
...
sub _initialize {
	...
	if ($select->rows == 1) {
	    ...
	    carp "Initializing eff_pathway_panther : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
