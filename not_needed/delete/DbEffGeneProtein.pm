package DbEffGeneProtein;

##########################################
##
## Document   : DbEffGeneProtein.pm
...
use constant ID_LOAD_STATEMENT => "SELECT * FROM eff_gene_protein WHERE id=?";

use constant GENE_LOAD_STATEMENT => "SELECT * FROM eff_gene_protein WHERE gene=?";
...
sub GetGeneProteinIdsForSearchTerm {
	my $search_term = shift;
	 my @ids = ();
	if ($search_term) {
		my $statement =  "SELECT id FROM eff_gene_protein WHERE gene LIKE '%$search_term%' OR description LIKE '%$search_term%' OR all_proteins LIKE '%$search_term%'";
		my $databaseHandle = DbSessionGpdb::GetConnection();
	        my $select = $databaseHandle->prepare($statement);
        	$select->execute();
	        my $nr = $select->rows();
...
sub GetAllGeneProteinsIds {
	my ($field_name, $asse_or_desc) = @_;
        my @ids = ();
        my $statement =  "";
		$statement = "SELECT DISTINCT gp.id FROM eff_gene_protein gp, eff_validation pv WHERE gp.gene = pv.ensembl_gene ORDER BY pv.$field_name $asse_or_desc";
        my $databaseHandle = DbSessionGpdb::GetConnection();
        my $select = $databaseHandle->prepare($statement);
        $select->execute();
        my $nr = $select->rows();
...
sub GetAllGenes {
        my @genes = ();
        my $statement =  "SELECT gene FROM eff_gene_protein";
        my $databaseHandle = DbSessionGpdb::GetConnection();
        my $select = $databaseHandle->prepare($statement);
        $select->execute();
        my $nr = $select->rows();
...
sub GetGeneDes {
	my $gene = shift;
        my $des = "";
        my $statement =  "SELECT description FROM eff_gene_protein where gene=?";
        my $databaseHandle = DbSessionGpdb::GetConnection();
        my $select = $databaseHandle->prepare($statement);
        $select->execute($gene);
...
sub GetGeneForProtein {
        my $protein = shift;
        my $gene = "";
        my $statement =  "SELECT gene FROM eff_gene_protein where longest_protein=?";
        my $databaseHandle = DbSessionGpdb::GetConnection();
        my $select = $databaseHandle->prepare($statement);
