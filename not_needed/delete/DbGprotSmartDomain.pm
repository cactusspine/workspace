...
use constant ID_LOAD_STATEMENT => "SELECT * FROM gprot_smart_domains WHERE id=?";
#use constant PROTEIN_LOAD_STATEMENT => "SELECT * FROM gprot_smart_domains WHERE protein=?";
...
sub GetProteinIdsForDomain {
	my $domain = shift;
        ...
        my $statement = "SELECT DISTINCT(protein_id) FROM gprot_smart_domains WHERE description=?";
...
sub GetAllProteinIds {
        my @proteinIds = ();
        my $databaseHandle = DbSessionGpdb::GetConnection();
        my $statement = "SELECT DISTINCT(protein_id) FROM gprot_smart_domains";
        my $select = $databaseHandle->prepare($statement);
        $select->execute();
        my $nr = $select->rows();
        while ($nr--) {
...
sub GetDomainIdsForPIDandDBName {
	...
	if ($protein_id) {
		my $databaseHandle = DbSessionGpdb::GetConnection();
		my $statement = "";
		$statement = "SELECT id FROM gprot_smart_domains WHERE protein_id=? ORDER BY start";
		my $select = $databaseHandle->prepare($statement);
...
sub GetDomainIdsForProtein {
        ...
        if ($protein) {
                my $databaseHandle = DbSessionGpdb::GetConnection();
                my $statement = "";
                $statement = "SELECT id FROM gprot_smart_domains WHERE protein=? ORDER BY start";
                my $select = $databaseHandle->prepare($statement);
...
#print "proteins : @proteins\n";
	...
	my $databaseHandle = DbSessionGpdb::GetConnection();
        ...
        foreach my $p (@proteins) {
                $statement = "SELECT DISTINCT(protein_id) FROM gprot_smart_domains WHERE protein=?";
		my $select = $databaseHandle->prepare($statement);
                $select->execute($p);
...
sub GetProteinForProteinId {
        ...
        $statement = "SELECT DISTINCT(protein) FROM gprot_smart_domains WHERE protein_id=?";
        my $select = $databaseHandle->prepare($statement);
        $select->execute($protein_id);
...
sub GetSmartProteinIdsDomainMaxMin {
	...
	my $databaseHandle = DbSessionGpdb::GetConnection();
        ...
        foreach my $p (@protein_ids) {
                $select = $databaseHandle->prepare("SELECT min(start), max(end) FROM gprot_smart_domains  WHERE protein_id=?");
                $select->execute($p);
...
sub getSmartDomainInfo {
        ...
        foreach my $d (@clusterDomainsIds) {
                my $domain = 0;
                $domain = new DbGprotSmartDomain($d);
                if ($domain->{Description}) {
                        push @domains, $domain;
                }
        } # foreach my $d (@clusterDomainsIds) {
