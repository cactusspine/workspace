package DbGpcrSummarySheet;

##########################################
##
## Document   : DbGpcrSummarySheet.pm
...
use constant ID_LOAD_STATEMENT => "SELECT * FROM gpcr_summary_sheet WHERE id=?";

use constant GENE_LOAD_STATEMENT => "SELECT * FROM gpcr_summary_sheet WHERE gene=?";
...
sub GetIdForHugo {
        my $hugo_name = shift;
        my $id = "";
        my $statement =  "SELECT id FROM gpcr_summary_sheet where hugo_name=?";
...
sub _initialize {
	...
	if ($select->rows == 1) {
	    ...
	    carp "Initializing gpcr_summary_sheet : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
