...
sub GetKeggForGene {
	...
	my $query = "";
	if ($org eq 'human') {
    $query = "SELECT * FROM biocompendium.human_pathway_kegg WHERE gene=?";
	}
	elsif($org eq 'mouse') {
		$query = "SELECT * FROM biocompendium.mouse_pathway_kegg WHERE gene=?;";
	}
	elsif($org eq 'yeast') {
		$query = "SELECT * FROM biocompendium.yeast_pathway_kegg WHERE gene=?;";
	}
	my $select = $databaseHandle->prepare("$query");
	$select->execute($gene);
	my $nr = $select->rows();
