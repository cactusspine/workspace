...
use constant ID_LOAD_STATEMENT => "SELECT * FROM yeast_smart_domains_corrected WHERE id=?";
#use constant PROTEIN_LOAD_STATEMENT => "SELECT * FROM yeast_smart_domains_corrected WHERE protein=?";
...
sub GetProteinIdsForDomain {
	my $domain = shift;
        ...
        my $statement = "SELECT DISTINCT(protein_id) FROM yeast_smart_domains_corrected WHERE description=?";
...
sub GetAllProteinIds {
        my @proteinIds = ();
        my $databaseHandle = DbSession::GetConnection();
        my $statement = "SELECT DISTINCT(protein_id) FROM yeast_smart_domains_corrected";
        my $select = $databaseHandle->prepare($statement);
        $select->execute();
        my $nr = $select->rows();
        while ($nr--) {
...
sub GetDomainIdsForPIDandDBName {
	...
	if ($protein_id) {
		my $databaseHandle = DbSession::GetConnection();
		my $statement = "";
		$statement = "SELECT id FROM yeast_smart_domains_corrected WHERE protein_id=? ORDER BY start";
		my $select = $databaseHandle->prepare($statement);
...
sub GetDomainIdsForProtein {
        ...
        if ($protein) {
                my $databaseHandle = DbSession::GetConnection();
                my $statement = "";
                $statement = "SELECT id FROM yeast_smart_domains_corrected WHERE protein=? ORDER BY start";
                my $select = $databaseHandle->prepare($statement);
...
sub getSmartDomainInfoForProtein {
        my ($protein) = @_;
        my @domainsIds = ();
        @domainsIds = &DbYeastSmartDomain::GetDomainIdsForProtein($protein);
        my @domains = ();
        foreach my $d (@domainsIds) {
                my $domain;
                $domain = new DbYeastSmartDomain($d);
...
#print "proteins : @proteins\n";
	...
	my $databaseHandle = DbSession::GetConnection();
        ...
        foreach my $p (@proteins) {
                $statement = "SELECT DISTINCT(protein_id) FROM yeast_smart_domains_corrected WHERE protein=?";
		my $select = $databaseHandle->prepare($statement);
                $select->execute($p);
...
sub GetProteinForProteinId {
        ...
        $statement = "SELECT DISTINCT(protein) FROM yeast_smart_domains_corrected WHERE protein_id=?";
        my $select = $databaseHandle->prepare($statement);
        $select->execute($protein_id);
...
sub GetSmartProteinIdsDomainMaxMin {
	...
	my $databaseHandle = DbSession::GetConnection();
        ...
        foreach my $p (@protein_ids) {
                $select = $databaseHandle->prepare("SELECT min(start), max(end) FROM yeast_smart_domains_corrected  WHERE protein_id=?");
                $select->execute($p);
...
#    }
	...
	if ($select->rows == 1) {
	    ...
	    carp "Initializing yeast_smart_domains_corrected : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
