...
use constant ID_LOAD_STATEMENT => "SELECT * FROM yeast_summary_sheet WHERE id=?";

use constant GENE_LOAD_STATEMENT => "SELECT * FROM yeast_summary_sheet WHERE gene=?";
...
sub _initialize {
	...
	if ($select->rows == 1) {
	    ...
	    carp "Initializing yeast_summary_sheet : unexpected number of records = ",$select->rows,"\n";
	}
    }
}
